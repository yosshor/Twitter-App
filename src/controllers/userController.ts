...
// Updated findUsersByUsername Controller
export const findUsersByUsername = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const usersWithFollowingStatus = users.map((user) => ({
      ...user.toObject(),
      isFollowing: following?.followingList.some((id) => id.toString() === user._id.toString()),
    }));
